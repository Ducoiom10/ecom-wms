<template>
    <div class="min-h-screen">
        ...
        <section class="relative h-96 overflow-hidden bg-indigo-700">
            <Transition name="fade" mode="out-in">
                <div :key="slideIdx" class="absolute inset-0">
                    <div
                        class="absolute inset-0 bg-gradient-to-r from-indigo-900/80 to-indigo-600/40"
                    />
                    <div
                    ...
                    >
                        <div>
                            <p
                                class="text-sm font-semibold uppercase tracking-widest mb-3 opacity-80"
                            >
                                {{ slides[slideIdx].tag }}
                            </p>
                            ...
                            <p class="text-lg mb-8 opacity-90 max-w-lg mx-auto">
        ...
        <section class="py-14 px-4 bg-white">
            <div class="max-w-7xl mx-auto">
                ...
                <div v-else class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <NuxtLink
                        v-for="cat in categories?.categories?.slice(0, 4)"
                        :key="cat.id"
                    ...
                    >
                        <div
                            class="absolute inset-0 bg-gradient-to-t from-indigo-900/60 to-transparent"
                        />
                        <h3
                            class="relative z-10 text-white font-bold text-lg text-center px-3"
                        >
                            {{ cat.name }}
                        </h3>
                    </NuxtLink>
        ...
        <section
            class="py-14 px-4 bg-gradient-to-r from-rose-500 to-orange-500"
        >
            <div class="max-w-3xl mx-auto text-center text-white">
                <p
                    class="text-xs font-bold uppercase tracking-widest mb-2 opacity-80"
                >
                    Ưu đãi có hạn
                </p>
                ...
                <p class="text-base mb-8 opacity-90">
                    Chỉ áp dụng cho sản phẩm được chọn. Nhanh tay kẻo hết!
                </p>
                <NuxtLink
                    to="/category"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    >
                        ...
                        <div class="flex items-center gap-3 pt-2 border-t">
                            <div
                                class="w-9 h-9 rounded-full bg-indigo-100 text-indigo-600 font-bold text-sm flex items-center justify-center"
                            >
                                {{ t.name[0] }}
                            </div>
                            <div>
        ...
        <section class="py-14 px-4 bg-indigo-600 text-white">
            <div class="max-w-xl mx-auto text-center">
                ...
                <p v-if="subMsg" class="mt-3 text-sm opacity-90">
                    {{ subMsg }}
                </p>
            </div>
        </section>
...
import type { CatalogFilters, Paginated, Product } from "~/types/models.types";
...
const slides = [
    ...
    {
        tag: "Flash Sale",
        title: "Giảm đến 50% hôm nay",
        subtitle: "Chỉ trong 24 giờ — số lượng có hạn",
        link: "/category",
        cta: "Xem ưu đãi",
    },
    {
        tag: "Hàng mới về",
        title: "Công nghệ tiên tiến 2026",
        subtitle: "Trải nghiệm sản phẩm công nghệ mới nhất",
        link: "/category",
        cta: "Khám phá ngay",
    },
];
...
// Data
const { data: categories, pending: catPending } =
    await useAsyncData<CatalogFilters>("home-categories", () =>
        useProductApi().getFilters(),
    );
const { data: products, pending: prodPending } = await useAsyncData<
    Paginated<Product>
>("home-products", () => useProductApi().getAll({ limit: 8 }));
...
const testimonials = [
    ...
    },
    {
        ...
        name: "Lê Văn C",
        rating: 4,
        comment: "Giá cả hợp lý, chất lượng ổn định.",
        date: "22/03/2026",
    },
];
const badges = [
    {
        ...
        desc: "Cho đơn hàng trên 500.000₫",
    },
    {
        icon: "🔒",
