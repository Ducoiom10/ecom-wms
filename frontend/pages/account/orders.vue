...
import type { Order, Paginated } from "~/types/models.types";
import { useOrderApi } from "~/composables/useOrderApi";

definePageMeta({ layout: "account", middleware: "auth" });
const filter = ref("");
const { data: orders, pending } = await useAsyncData<Paginated<Order>>(
    "my-orders",
    () => useOrderApi().getOrders(1, 50),
);
const filtered = computed(() => {
    const list = orders.value?.data ?? [];
    return filter.value
        ? list.filter((order: Order) => order.status === filter.value)
        : list;
});
