    ...
    <div class="bg-white rounded-xl border p-6 space-y-5">
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div v-for="p in wishlist" :key="p.id" class="relative">
                ...
                <button
                    @click="remove(p.id)"
                    class="absolute top-2 right-2 bg-white rounded-full w-7 h-7 flex items-center justify-center shadow text-red-500 hover:bg-red-50"
                >
                    ×
                </button>
            </div>
...
const wishlist = computed<readonly Product[]>(() => wishlistStore.items);
