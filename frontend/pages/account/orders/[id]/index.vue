    ...
    <div class="bg-white rounded-xl border p-6 space-y-6">
        ...
        <div v-if="pending" class="space-y-3">
            <div
                v-for="i in 3"
                :key="i"
                class="h-20 bg-gray-100 rounded-lg animate-pulse"
            />
        </div>
        <div v-else class="border rounded-xl overflow-hidden">
            <div class="px-4 py-3 bg-gray-50 border-b">
                <h2
                    class="font-semibold text-sm text-gray-600 uppercase tracking-wide"
                >
                    Sản phẩm
                </h2>
            </div>
            <div class="divide-y">
                ...
                >
                    <div
                        class="w-14 h-14 bg-gray-100 rounded-lg flex-shrink-0 overflow-hidden"
                    >
                        <img
                            v-if="item.product?.productImages?.[0]?.image_url"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="border rounded-xl p-4 space-y-2 text-sm">
                <h2
                    class="font-semibold text-gray-600 uppercase tracking-wide text-xs mb-3"
                >
                    Tổng tiền
                </h2>
                ...
                <div class="flex justify-between text-gray-600">
                    <span>Thuế</span><span>{{ fmt(order?.tax ?? 0) }}</span>
                </div>
                <div
            ...
            <div class="border rounded-xl p-4 space-y-2 text-sm">
                ...
                >
                    ...
                    <span
                        class="bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs font-mono"
                ...
                <div class="pt-2 border-t space-y-1 text-xs text-gray-400">
                    ...
                    <p v-if="order?.approved_at">
                        Duyệt lúc: {{ fmtDate(order.approved_at) }}
                    </p>
                    <p v-if="order?.shipped_at">
                        Giao lúc: {{ fmtDate(order.shipped_at) }}
                    </p>
                    <p v-if="order?.delivered_at">
                        Nhận lúc: {{ fmtDate(order.delivered_at) }}
                    </p>
                </div>
            </div>
        </div>
...
const {
    ...
    pending,
    refresh,
} = await useAsyncData<Order>(`order-detail-${route.params.id}`, () =>
...
const isCancellable = (status?: string) =>
    ["pending", "approved", "picking", "picked", "packed"].includes(
        status ?? "",
    );
