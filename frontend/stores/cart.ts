...
export const useCartStore = defineStore('cart', () => {
  ...
  const fetchCart = async () => {
    loading.value = true
    try { cart.value = await useCartApi().getCart() }
  ...
  const updateQuantity = async (productId: number, quantity: number, variantId?: number) => {
    loading.value = true
    try { cart.value = await useCartApi().updateItem(productId, quantity, variantId) }
    finally { loading.value = false }
  }

  const removeItem = async (productId: number, variantId?: number) => {
    loading.value = true
    try { cart.value = await useCartApi().removeItem(productId, variantId) }
    finally { loading.value = false }
  }
