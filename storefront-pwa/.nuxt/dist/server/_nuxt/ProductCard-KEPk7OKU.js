...
import { ref, readonly, defineComponent, computed, mergeProps, unref, withCtx, createVNode, toDisplayString, useSSRContext } from "vue";
import { ssrRenderAttrs, ssrRenderClass, ssrRenderComponent, ssrRenderAttr, ssrInterpolate, ssrIncludeBooleanAttr } from "vue/server-renderer";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const primaryImage = computed(
      () => props.product.productImages?.find((i) => i.is_primary)?.image_url ?? props.product.productImages?.[0]?.image_url ?? "/placeholder.png"
    );
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="px-3 pb-3"><button${ssrIncludeBooleanAttr(unref(cartStore).loading) ? " disabled" : ""} class="w-full text-xs bg-indigo-600 text-white py-1.5 rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition"> Thêm vào giỏ </button></div></div>`);
    };
  }
});
...
//# sourceMappingURL=ProductCard-KEPk7OKU.js.map
