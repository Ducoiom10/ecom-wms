<template>
  <header class="sticky top-0 z-40 bg-white border-b shadow-sm">
    <div class="max-w-7xl mx-auto px-4 h-16 flex items-center gap-4">
      ...
      <!-- Logo -->
      <NuxtLink to="/" class="text-xl font-bold text-indigo-600 flex-shrink-0">EcomWMS</NuxtLink>

      <!-- Nav links (desktop) -->
      <nav class="hidden md:flex items-center gap-1 flex-1">
        <NuxtLink to="/" class="px-3 py-2 text-sm text-gray-600 hover:text-indigo-600 rounded-lg hover:bg-gray-50 transition">
          Trang chủ
        </NuxtLink>
        <NuxtLink to="/category" class="px-3 py-2 text-sm text-gray-600 hover:text-indigo-600 rounded-lg hover:bg-gray-50 transition">
          Danh mục
        </NuxtLink>
      </nav>
      ...
      <div class="flex-1 max-w-sm relative hidden md:block">
        <input v-model="searchQ" @keydown.enter="doSearch" type="text" placeholder="Tìm sản phẩm..."
        ...
        <div v-if="searchResults.length" class="absolute top-full left-0 right-0 mt-1 bg-white border rounded-xl shadow-lg z-50 overflow-hidden">
          <NuxtLink v-for="p in searchResults" :key="p.id" :to="`/products/${p.id}`"
            @click="searchQ = ''; searchResults = []"
            class="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition text-sm">
            <span class="font-medium truncate">{{ p.name }}</span>
            <span class="ml-auto text-red-600 font-semibold flex-shrink-0">
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <!-- Cart -->
        <button @click="uiStore.toggleCart()" class="relative p-2 hover:bg-gray-100 rounded-lg transition">
        ...
        <template v-if="!authStore.isLoggedIn">
          <NuxtLink to="/login" class="text-sm font-medium text-indigo-600 hover:underline px-2">Đăng nhập</NuxtLink>
          <NuxtLink to="/register" class="text-sm bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition hidden md:block">
            Đăng ký
          </NuxtLink>
        </template>
        ...
        <div v-else class="relative" ref="dropdownRef">
          <button @click="dropdownOpen = !dropdownOpen"
            ...
            <div class="w-7 h-7 rounded-full bg-indigo-100 text-indigo-600 font-bold text-xs flex items-center justify-center">
              {{ initials }}
            </div>
            <span class="hidden md:block font-medium max-w-24 truncate">{{ authStore.user?.name }}</span>
          ...
          <Transition enter-from-class="opacity-0 scale-95" enter-active-class="transition duration-150"
            ...
            <div v-if="dropdownOpen"
              ...
              <NuxtLink v-for="item in accountMenu" :key="item.to" :to="item.to"
...
watch(searchQ, (q) => {
  ...
  searchTimer = setTimeout(async () => {
    try {
      const res = await useProductApi().search(q, 5) as any
